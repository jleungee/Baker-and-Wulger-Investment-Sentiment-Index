...
# Getting the Baker and Wulger components
bw = pd.read_csv("C:\\Users\\jeffr\\Desktop\\FINA 4392\\BandW\\bw.csv")
bwm = reduce(lambda x,y: pd.merge(x,y,on="month"),
             [bw[["month","Sm"]].groupby("month").mean(),
              bw[["RIPO","NIPO","month"]].groupby("month").sum(),
...
results = smf.ols(
    ...
    data=bw_ref.iloc[93:,:]).fit(cov_type = "HC1")
...
# There are only 18 matching observations but still return ~85% R^2
bw_reg = pd.concat([
