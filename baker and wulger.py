...
IPO = pd.merge(
    ...
    IPO,
    on="Date",
    how = "left").drop(["Date"],axis=1).groupby("week").sum()
...
pdnd = pdnd.groupby("week").mean()




# Constructio of the Baker and Wulger Index
bw = reduce(lambda x,y: pd.merge(x,y,on="week"),[cef,IPO,S,pdnd])
bw.to_csv("C:\\Users\\jeffr\\Desktop\\FINA 4392\\BandW\\bw.csv")
